const CheckboxItem = ({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) => {
    return (
        <label className="flex items-center gap-2 cursor-pointer group mb-2 last:mb-0">
            <div onClick={onChange} className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all flex-shrink-0 ${checked ? "bg-sky-600 border-sky-600" : "border-gray-300 group-hover:border-sky-400"}`}>
